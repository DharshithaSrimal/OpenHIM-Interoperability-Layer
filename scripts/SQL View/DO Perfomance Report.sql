...
SELECT
	...
	COUNT(DISTINCT CASE WHEN (psi.status = 'ACTIVE' AND ps.programstageid = 495 AND date_trunc('week', CURRENT_DATE) = date_trunc('week', psi.completeddate)) THEN psi.uid ELSE NULL END) AS SCREENINGS_DUE,
	...
	COUNT(DISTINCT CASE WHEN (psi.status = 'ACTIVE' AND ps.programstageid = 500 AND date_trunc('day', CURRENT_DATE) BETWEEN date_trunc('day', psi.completeddate) + INTERVAL '14 day' AND date_trunc('day', CURRENT_DATE) - INTERVAL '1 day')) THEN psi.uid ELSE NULL END) AS PHONE_CALLS_DUE,
	COUNT(DISTINCT CASE WHEN (psi.status = 'ACTIVE' AND ps.programstageid = 500 AND date_trunc('day', CURRENT_DATE) < date_trunc('day', psi.completeddate) + INTERVAL '14 day')) THEN psi.uid ELSE NULL END) AS PHONE_CALLS_OVERDUE,
...
GROUP BY
    ui.surname, ui.firstname, tei.createdbyuserinfo->>'username', ur.name;
