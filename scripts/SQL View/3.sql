 ...
 SELECT ou.name AS org_unit,
    ...
    count(DISTINCT
        CASE
            WHEN psi.status::text = 'ACTIVE'::text AND ps.name::text = 'Follow - up Status'::text AND ((psi.eventdatavalues -> 'VCQ4bYBggPB'::text) ->> 'value'::text) = 'Screening_Completed'::text AND (psi.executiondate + '14 days'::interval) < date_trunc('day'::text, CURRENT_DATE::timestamp with time zone) THEN psi.uid
            ELSE NULL::character varying
    ...
    count(DISTINCT
        CASE
            WHEN psi.status::text = 'ACTIVE'::text AND ps.name::text = 'Follow - up Status'::text AND ((psi.eventdatavalues -> 'VCQ4bYBggPB'::text) ->> 'value'::text) = 'Phone_Calls_Completed'::text AND (psi.executiondate + '14 days'::interval) < date_trunc('day'::text, CURRENT_DATE::timestamp with time zone) THEN psi.uid
            ELSE NULL::character varying
