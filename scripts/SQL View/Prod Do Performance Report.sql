...
SELECT 
	...
	COUNT(DISTINCT CASE WHEN (psi.eventdatavalues->'vjABPom3WZD'->>'value' = 'false' AND psi.status = 'COMPLETED' AND ps.name = 'Screening at the community') THEN psi.uid ELSE NULL END) AS SCREENING_NOT_REQUIRED,
    ...
    COUNT(DISTINCT CASE WHEN (psi.eventdatavalues->'vjABPom3WZD'->>'value' = 'true' AND psi.eventdatavalues->'igk7wHUjNoY'->>'value' = 'true' AND psi.status = 'COMPLETED' AND ps.name = 'Screening at the community' AND psi.eventdatavalues != '{}') THEN psi.uid ELSE NULL END) AS CLIENTS_SCREENED,
