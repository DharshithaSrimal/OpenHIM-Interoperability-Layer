...
LOG_FILE_PATH = "tei_errors_del.log"
# FHIR
...
encounters = requests.get(
    f"{FHIR_SERVER_URL}/fhir/Encounter?_sort=_lastUpdated&_count=400000&_lastUpdated=ge{START_DATE}&date=le{END_DATE}&_tag=2.1.0-diabetesCompass,2.1.1-diabetesCompass,2.0.2-diabetesCompass,2.0.0-diabetesCompass&type=184047000",
    headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
)

# encounters = requests.get(
#     f"{FHIR_SERVER_URL}/fhir/Encounter?_count=400000&date=ge{START_DATE}&date=le{END_DATE}&_tag=PKT0010405&type=184047000",
#     headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
# )
...
for encounter_info in encounters_bundle:
    ...
    encounter_resource = encounter_info["resource"]
    # Extract the Practitioner Location code
    practitioner_location_code = next((tag["code"] for tag in encounter_resource["meta"]["tag"] if tag["system"] == "https://smartregister.org/location-tag-id"), None)
    ...
    # if tei_id == "":
        ...
        try:
                ...
                mapping_response = requests.post(
                    "http://localhost:5001/fhir-dump-to-tei",
                ...
                # if phn != None:
                    ...
                    if tei_response_json['trackedEntityInstances']:
                        try:
                            endpointt = 'trackedEntityInstances/'+ tei_id
                            print(endpointt)
                            tei_put_response = api.delete(endpoint=endpointt)
                            # Remove attributes without a "value" key
                            # mapping_result["attributes"] = [attr for attr in mapping_result["attributes"] if "value" in attr]
                            # tei_put_response = api.put('trackedEntityInstances/'+ tei_id, json=mapping_result)
                            if tei_put_response.status_code != 200:
                                print(f"TEI delete failed: ", tei_put_response.json(), " Encounter: ", encounter_id)
                            else:
                                print("Success")
                        except Exception as e:
                            print(f"TEI update Failed: {str(e)}")
                            error_message = f"TEI update Failed: {str(e)}, Encounter: {encounter_id}"
                            log_error(error_message)
