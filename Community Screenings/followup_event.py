...
service_request = requests.get(
    f"{FHIR_SERVER_URL}/fhir/ServiceRequest?status={status[2]}&_count=0&_tag=2.0.0-diabetesCompass,2.0.2-diabetesCompass,2.1.0-diabetesCompass,2.1.1-diabetesCompass",
...
if (service_request_count > 0):
    ...
    service_request_response = requests.get(
        f"{FHIR_SERVER_URL}/fhir/ServiceRequest?status=completed&_count={service_request_count}&_tag=2.0.0-diabetesCompass,2.0.2-diabetesCompass,2.1.0-diabetesCompass,2.1.1-diabetesCompass",
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
...
tasks = requests.get(
    f"{FHIR_SERVER_URL}/fhir/Task?status={status[2]}&_count=0&identifier=phone_call_follow_up&_tag=2.0.0-diabetesCompass,2.0.2-diabetesCompass,2.1.0-diabetesCompass,2.1.1-diabetesCompass",
    headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
)
...
if (tasks_count > 0):
    ...
    task_response = requests.get(
        # status={status[2]}&
        f"{FHIR_SERVER_URL}/fhir/Task?_count={tasks_count}&identifier=phone_call_follow_up&_tag=2.0.0-diabetesCompass,2.0.2-diabetesCompass,2.1.0-diabetesCompass,2.1.1-diabetesCompass",
        headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
    )
            ...
            try:
                ...
                try:
                    ...
                    if tei_event_response.status_code == 200:
                        print("Phone Call Event Update success")
                    else:
                        print(f"Phone Call Event Update Failed")
                except Exception as e:
