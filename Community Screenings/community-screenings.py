...
PHN = config["PHN"]
NIC = config["NIC"]
CALLING_NAME = config["CALLING_NAME"]
...
PATIENT_ORIGIN = config["PATIENT_ORIGIN"]

LOG_FILE_PATH = "tei_log_errors.log"
...
# patients_response = requests.get(
#     f"{FHIR_SERVER_URL}/fhir/Patient?_count=1000&_lastUpdated=ge{START_DATE}&_lastUpdated=le{END_DATE}&_tag=gnd-700,gnd-705,706A",
#     headers={"Authorization": f"Bearer {ACCESS_TOKEN}"}
# )
...
for patient_info in patients_bundle:
    ...
    phn = next((tag["value"] for tag in patient_resource["identifier"] if tag["type"]["coding"]["code"] == "PHN"), None)
    nic = next((tag["value"] for tag in patient_resource["identifier"] if tag["type"]["coding"]["code"] == "NIC"), None)
    ...
    rr_result = {
            ...
            "attributes": [
                ...
                {
                "attribute": PATIENT_ORIGIN,
                "value":data_origin
                },
                {
                "attribute": PRACTITIONER,
                "value":practitioner_code
                }
            ],
            "enrollments": [
                {
                "enrollmentDate": "2025-06-01T14:28:59.005+00:00",
    ...
    if mapping_response.status_code == 200:
        ...
        try:
            ...
            encounter_info = encounter_response_bundle[0]
            encounter_date = encounter_info["resource"]["period"]["start"]
            event_date = encounter_date
            # Add incidentDate to enrollments
            mapping_result["enrollments"][0]["incidentDate"] = encounter_date
            mapping_result["enrollments"][0]["enrollmentDate"] = encounter_date
        except Exception as e:
...
if questionnaireResponse.status_code == 200:
    ...
    for entry in qr_bundle:
        ...
        try:
            ...
            error_message = f"Community mapping error: {e}, Error: {post_response.json()}"
            log_error(error_message)
            continue
